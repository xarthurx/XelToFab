---
title: load_density
description: Load a density field from file into a PipelineState
---
...
Returns a `PipelineState` with the loaded `density` array and the given `params`.
...
```python
state = load_density("fields.npz", field_name="density")
```

### MATLAB file

```python
state = load_density("topology_result.mat", field_name="xPhys")
```

### CSV with reshape

```python
state = load_density("flat_field.csv", shape=(100, 200))
```

### HDF5 file

```python
state = load_density("simulation.h5", field_name="density")
```
