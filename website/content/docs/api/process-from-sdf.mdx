...
This reduces evaluations from O(N³) to ~O(N²). Unevaluated regions are filled with `+1.0` (outside), so extraction ignores them.
...
See the [SDF Functions guide](/docs/guides/sdf-functions#uniform-vs-adaptive-evaluation) for details on when adaptive evaluation helps.
...
- [SDF Functions Guide](/docs/guides/sdf-functions) — when to use, wrapping models, uniform vs adaptive
