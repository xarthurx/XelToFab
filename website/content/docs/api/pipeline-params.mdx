...
<Callout type="info" title="Using with process_from_sdf()">
When using [`process_from_sdf()`](/docs/api/process-from-sdf), `field_type="sdf"` is set automatically — you don't need to specify it. Any additional `PipelineParams` fields can be passed as keyword arguments (e.g., `process_from_sdf(my_sdf, bounds, smoothing_method="taubin")`).
</Callout>
