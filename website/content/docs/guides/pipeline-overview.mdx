...
XelToFab processes design optimization output through six stages:
...
Isotropic remeshing via [`gpytoolbox.remesh_botsch`](https://gpytoolbox.org/) (Botsch & Kobbelt algorithm). Replaces the marching cubes triangulation with uniform, well-shaped triangles. After remeshing, 99%+ of faces meet FEA quality targets (min angle >20°).
...
A mesh extracted from a 3D heat conduction topology optimization result (51×51×51 density field, processed with default pipeline parameters — 7,087 vertices, 13,910 faces):

<MeshViewer src="/models/heat_cond_51_s0.stl" height={500} cameraDistance={15} fov={35} />

A synthetic sphere generated from an 80×80×80 density field (836 vertices, 1,668 faces after full pipeline processing):
