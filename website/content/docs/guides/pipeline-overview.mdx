...
+--------------+
       ...
       v
+--------------+
|   Repair      |  3D: fix non-manifold edges/vertices (pymeshlab)
|               |  2D: no-op
+--------------+
       |  repaired mesh
       v
+--------------+
|   Remesh      |  3D: isotropic remeshing (gpytoolbox, Botsch & Kobbelt)
|               |  2D: no-op
+--------------+
       |  uniform triangle mesh
       v
+--------------+
|  Decimate     |  3D: QEM edge collapse (pyfqmr)
|               |  2D: no-op
+--------------+
       |  optimized mesh
       v
...
Isotropic remeshing via `gpytoolbox.remesh_botsch` (Botsch & Kobbelt algorithm). Replaces the marching cubes triangulation with uniform, well-shaped triangles. After remeshing, 99%+ of faces meet FEA quality targets (min angle >20°).
