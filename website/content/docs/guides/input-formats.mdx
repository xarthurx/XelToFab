---
title: Input Formats
description: All supported file formats for loading density and SDF fields
---
...
state = load_density("field.npy")

# .npz -- uses "density" key if present, otherwise the first key
state = load_density("field.npz")

# .npz -- specify which array to load
state = load_density("field.npz", field_name="xPhys")
```
...
Loads `.mat` files via `scipy.io.loadmat`. The loader auto-detects common topology optimization variable names in priority order:
...
```python
state = load_density("field.csv", shape=(50, 100))       # 2D
state = load_density("field.csv", shape=(10, 20, 30))    # 3D
```
...
The loader auto-detects density fields by looking for cell data or point data arrays with names matching common TO patterns (`density`, `rho`, `xphys`, `x`), checking cell data first (since TO densities are per-element). If no match is found and only one data array exists, it is used automatically.

```python
state = load_density("result.vtk")
state = load_density("result.vti", field_name="density")
```
...
Auto-detection searches all datasets recursively, checking leaf names against the same known TO variable names as the MATLAB loader (`xPhys`, `densities`, `x`, `rho`, `dc`, `density`).

```python
state = load_density("result.h5")
state = load_density("result.h5", field_name="simulations/density")
```
...
```python
state = load_density("result.xdmf")
state = load_density("result.xdmf", field_name="density")
```
...
All loaders accept these parameters via `load_density()`:
