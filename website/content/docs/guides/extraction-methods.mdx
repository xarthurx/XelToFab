...
DC extraction supports GPU acceleration via the [isoext](https://github.com/GuangyanCai/isoext) backend, which is auto-detected when available.

**Benchmark** (measured on NVIDIA RTX 5080 — results will vary by GPU):

| Grid | GPU | CPU | Speedup |
|------|-----|-----|---------|
| 100^3 | 0.021s | 0.380s | 18x |
| 500^3 | 1.36s | 38.68s | 28x |

Install the GPU backend:

```bash
uv sync --extra cuda  # requires CUDA toolkit + PyTorch
```

If the GPU is unavailable, DC automatically falls back to the CPU implementation with a warning:
...
Cube extracted with Marching Cubes (left) vs Dual Contouring (right). Notice how DC preserves the sharp edges and corners:

<div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
  <div>
    <p className="text-center font-semibold mb-2">Marching Cubes</p>
    <MeshViewer src="/models/cube_mc.stl" height={350} cameraDistance={40} fov={35} />
  </div>
  <div>
    <p className="text-center font-semibold mb-2">Dual Contouring</p>
    <MeshViewer src="/models/cube_dc.stl" height={350} cameraDistance={40} fov={35} />
  </div>
</div>
