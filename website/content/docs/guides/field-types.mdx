---
title: Field Types
description: Density fields vs SDF fields and how field_type affects pipeline behavior
---

## Overview

XelToFab supports two types of input fields, configured via the `field_type` parameter:

| Field type | Typical source | Value range | Extraction level |
|------------|---------------|-------------|-----------------|
| `density` (default) | Classical TO solvers (SIMP, BESO), occupancy networks | [0, 1] | 0.5 (at `threshold`) |
| `sdf` | Neural SDF models (DeepSDF, NITO, NTopo) | unbounded (negative inside, positive outside) | 0.0 (zero level set) |

## Density fields

Density fields contain continuous values in [0, 1] where higher values indicate material presence. This is the standard output format from topology optimization solvers.
...
- **`smooth_sigma = 1.0`** -- Gaussian smoothing helps reduce noise in the density field before binarization
- **`direct_extraction = False`** -- Extraction operates on the preprocessed binary field
- **Extraction level = `threshold`** (default 0.5) -- The marching cubes/squares isovalue

This pipeline works well for raw TO output where density values may be noisy or have intermediate values that need cleanup.
...
- **Taubin smoothing still runs** -- Even with clean SDF input, marching cubes produces staircase artifacts on the mesh that benefit from Taubin smoothing
...
The extraction level is the isovalue passed to marching cubes (3D) or `find_contours` (2D). It determines which isosurface in the field becomes the mesh boundary.

The default depends on field type:

| Field type | Default extraction level | Reasoning |
|------------|------------------------|-----------|
| `density` | Value of `threshold` (default 0.5) | The 50% density contour separates material from void |
...
- Your input comes from a classical TO solver (SIMP, BESO, evolutionary methods)
