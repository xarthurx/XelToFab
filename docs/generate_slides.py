...
def add_title(slide, text, left=0.8, top=0.4, width=8.4, height=1.0, font_size=32, color=WHITE, bold=True):
...
def add_subtitle(slide, text, left=0.8, top=1.3, width=8.4, height=0.6, font_size=18, color=SUBTITLE_CLR):
    return add_title(slide, text, left, top, width, height, font_size, color, bold=False)


def add_body_text(
    slide, lines, left=0.8, top=2.0, width=8.4, height=4.5, font_size=16, color=BODY_CLR, line_spacing=1.4
):
    txBox = slide.shapes.add_textbox(Inches(left), Inches(top), Inches(width), Inches(height))
    tf = txBox.text_frame
    tf.word_wrap = True
...
def add_accent_bar(slide, top=1.15):
    shape = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.8), Inches(top), Inches(2.0), Inches(0.04))
    shape.fill.solid()
...
def add_flow_arrow(slide, labels, top=2.0, colors=None):
    ...
    for i, label in enumerate(labels):
        ...
        # Box
        shape = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(x), Inches(top), Inches(box_w), Inches(0.7))
        shape.fill.solid()
        ...
        if i < n - 1:
            ...
            arrow = slide.shapes.add_shape(
                MSO_SHAPE.RIGHT_ARROW, Inches(ax + 0.02), Inches(top + 0.22), Inches(gap - 0.04), Inches(0.26)
            )
            arrow.fill.solid()
...
def add_table(slide, headers, rows, left=0.8, top=2.2, col_widths=None):
    ...
    table_shape = slide.shapes.add_table(
        n_rows, n_cols, Inches(left), Inches(top), Inches(sum(col_widths)), Inches(0.35 * n_rows)
    )
...
add_subtitle(slide, "From Density Fields to Fabrication-Ready Geometry", top=2.0, font_size=22, color=WHITE)
add_body_text(
    slide,
    [
...
add_flow_arrow(
    slide, ["TO Solver", "Density\nField", "???", "Usable\nGeometry"], top=4.2, colors=[GREEN, ACCENT, RED_ACCENT, GRAY]
)
...
add_accent_bar(slide)
add_flow_arrow(
    slide, ["Preprocess", "Extract", "Post-process", "Export"], top=1.7, colors=[GREEN, GREEN, YELLOW, GREEN]
)
add_body_text(
    slide,
    [
...
for i, label in enumerate(["Raw Density", "Binary", "Mesh"]):
    x = 1.5 + i * 2.6
    shape = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(x), Inches(3.4), Inches(2.2), Inches(1.6))
    shape.fill.solid()
