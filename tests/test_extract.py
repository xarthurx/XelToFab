...
def test_extract_2d_produces_contours(circle_field: np.ndarray):
    state = preprocess(PipelineState(field=circle_field))
...
def test_extract_3d_produces_nonempty_mesh(sphere_field: np.ndarray):
    state = preprocess(PipelineState(field=sphere_field))
    result = extract(state)
...
def test_extract_requires_preprocessing(circle_field: np.ndarray):
    state = PipelineState(field=circle_field)
...
def test_extract_2d_direct_sdf(circle_sdf: np.ndarray):
    ...
    params = PipelineParams(field_type="sdf")
    state = PipelineState(field=circle_sdf, params=params)
    result = extract(state)
    assert result.contours is not None
    assert len(result.contours) > 0


def test_extract_3d_direct_density(sphere_field: np.ndarray):
    """Direct extraction from clean density field at level=0.5."""
    params = PipelineParams(field_type="density", direct_extraction=True)
    state = PipelineState(field=sphere_field, params=params)
    result = extract(state)
    assert result.vertices is not None
    assert result.faces is not None
    assert result.vertices.shape[0] > 0
